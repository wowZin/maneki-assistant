...
## 同花顺 Cookie 直连 (2个接口，复用为多缓存)

> 数据优先级: **同花顺Cookie直连(ths_client) > Tushare**。实时数据优先走同花顺Cookie直连，Tushare仅作兜底。
> 方式: **同花顺Cookie直连**。配置: `.env` 中 `THS_COOKIE`, 模块 `ths_client.py` 提供统一接口。

| 接口 | 用途 | 关键字段 | 已知坑 |
|------|------|------|------|
| `ths_client.get_realtime_quotes` | 异动扫描/行情/资金流/人气 | f3(涨幅) f11(涨速) f62(主力净流入) f10(量比) f6(成交额) f7(换手率) | 需要有效Cookie，过期需刷新 |
| `ths_client.get_stock_realtime` | 个股实时行情(分时数据) | f170(涨跌幅) | secid格式:`{market}.{code}` |

> 三个缓存复用同一接口: `_get_realtime_fund_cache`(f62/f10/f7/f6) + `_get_popularity_rank`(f62) + `_get_realtime_pct_cache`(f3)
...
| 同花顺Cookie直连超时/空 | Cookie过期 | 刷新Cookie |
| call_tushare静默失败 | except:pass吞错误 | 看return是否为空dict |

## akshare 接口 (5个)

> ⚠️ akshare 底层基于已被TCP层封禁的API，**akshare 大部分接口不可用**。
