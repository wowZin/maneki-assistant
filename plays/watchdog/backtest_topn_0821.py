#!/usr/bin/env python3
"""回测：每日 model_score top10 → 开盘买入 → 次日开盘卖（用户 0821 要求）"""
import json
import requests
...
for topn in (5, 10, 20):
    ...
    for i, d in enumerate(days):
        ...
        top = sorted(t, key=lambda x: -(x.get("model_score") or 0))[:topn]
        ...
        for r in top:
            ...
            tr.append({"day": d, "code": c,
                       "panel": round(r.get("model_score") or 0, 1),
                       "gap": round(gap * 100, 1), "pnl": pnl})
    if not tr:
        continue
    n = len(tr)
    ...
    print(f"Top{topn}: {n}笔 均{avg:+.2f}% 胜率{wins/n*100:.0f}% "
    ...
    # 最差 5 笔明细
    print("    最差5笔:")
    for x in sorted(tr, key=lambda y: y["pnl"])[:5]:
        print(f"      {x['day']} {x['code']} panel={x['panel']} gap={x['gap']}% {x['pnl']:+.1f}%")
