...
def is_tradable_stock(code: str, name: str = "") -> bool:
    """判断股票是否属于可交易标的（主板 + 非 ST/退市/新股）。

    过滤规则：
    - 排除 ST、*ST、退市、N 开头等异常状态
    - 排除创业板(300/301)、科创板(688)、北交所(8/4/920)

    Args:
        code: 带后缀的股票代码，如 "000001.SZ"。
        name: 股票名称，用于 ST/退市判断。
    ...
    if not code:
        return False
    if name and any(x in name for x in ("ST", "*ST", "退", "N")):
        return False
    pure = code.split(".")[0]
