...
# 跨评分调用的 Tushare 缓存（同一天多只股共享）
_TUSHARE_CACHE: dict[str, dict] = {}


def _make_patcher(target_date: str):
    """patch call_tushare，将 date 参数替换为目标日期 + 全局结果缓存"""
    from scripts import tu_share as ts_mod
    orig = ts_mod.call_tushare

    def _cache_key(api: str, params: dict) -> str:
        clean = {k: v for k, v in sorted(params.items()) if v is not None}
        return f"{api}:{json.dumps(clean, sort_keys=True)}"

    def patched(api: str, params: dict, fields: str = ""):
        np = dict(params)
        for k in ("trade_date", "start_date", "end_date"):
            if k in np:
                np[k] = target_date
        key = _cache_key(api, np)
        if key in _TUSHARE_CACHE:
            return _TUSHARE_CACHE[key]
        result = orig(api, np, fields)
        _TUSHARE_CACHE[key] = result
        return result

    return patch.object(ts_mod, "call_tushare", patched)


def _score_one_stock(code: str, date: str) -> dict | None:
    """对一只股票跑五维度评分，patch call_tushare 使 Tushare 查询指向历史日期"""
    with _make_patcher(date):
